...
import { spawnSly } from "./spawnSly"

import { rmSync } from "fs"

describe.each(supportedVersions)(`%s`, (version) => {
  ...
  test.skipIf(isBelowVersion("1.4.6"))(
    ...
    async (test) => {
      const { waitForText } = await spawnSly(test, `add`)
  ...
  test.skipIf(isBelowVersion("1.4.6"))(
    ...
    async (test) => {
      const { waitForText } = await spawnSly(
        test,
        `add @radix-ui/icons eraser --directory ./tests/temp/${test.task.id}/ --overwrite --yes`
      )
  ...
  test.skipIf(isBelowVersion("1.4.6"))(
    ...
    async (test) => {
      const { waitForText } = await spawnSly(
        test,
        `add @radix-ui/icons camera card-stack --directory ./tests/temp/${test.task.id}/ --overwrite --yes`
      )
  ...
  test.skipIf(isBelowVersion("1.4.6"))("adds icon from arg", async (test) => {
    const { waitForText, waitForFinish, writeText, cleanup, readFile } =
      await spawnSly(
        test,
        `add @radix-ui/icons eraser --directory ./tests/temp/${test.task.id}/ --overwrite`
      )
  ...
  test.skipIf(isBelowVersion("1.4.6"))("adds multiple icons", async (test) => {
    const { waitForText, waitForFinish, writeText, cleanup, readFile } =
      await spawnSly(
        test,
        `add @radix-ui/icons camera card-stack --directory ./tests/temp/${test.task.id}/ --overwrite`
      )
  ...
  test.skipIf(isBelowVersion("1.4.6"))(
    ...
    async (test) => {
      ...
      const { waitForText, waitForFinish, writeText, cleanup } = await spawnSly(
        test,
        `add @radix-ui/icons eraser --directory ./tests/temp/${test.task.id}/`
      )
